...
# ================================================================
# Each takes the game state and agent color, returns a scalar.
# These are computed as snapshots/deltas per transition.
...
def network_position_score(state, color):
    ...
    - +0.2 * pips per specific port connected
    ...
    - +0.3 per knight played (positional: robber control + army progress)
    """
    board = state.board
    key = player_key(state, color)
    score = 0.0
    ...
    # Track accessible resource types and pip coverage
    ...
    for node_id, (bldg_color, bldg_type) in buildings.items():
        ...
        for tile in tiles:
            ...
            if tile.coordinate == robber_coordinate:
                continue  # zeroed if robber is here
    ...
    # Port access
    # board.map.port_nodes is Dict[resource | None, Set[node_ids]]
        ...
        for resource, node_ids in board.map.port_nodes.items():
            if any(nid in buildings and buildings[nid][0] == color for nid in node_ids):
                if resource is None:  # generic 3:1 port
                    ...
                    # Specific port: +0.2 * pips of that resource accessible
                    # Simplified: use flat 0.4 for specific ports
                    score += 0.4
    except (AttributeError, TypeError):
        pass  # port access not available in this state representation
    ...
    # Open settlement spots (simplified: count buildable nodes)
    try:
...
def vp_proximity_reward(state, color, prev_vps, prev_knights,
                        opp_color):
    """
    ...
    if new_knights > 0:
        ...
        if achievable:
            ...
            # Scale by how reasonable the target is
...
def terminal_reward(game, color):
    ...
    - Win: +10 + 0.5 * (your_VPs - opponent_VPs)
    - Loss: -10 + 0.5 * (your_VPs - opponent_VPs)
    """
...
class CatanRewardWrapper(Wrapper):
    ...
    def __init__(self, env):
        super().__init__(env)
        self.p0_color = Color.BLUE  # agent color
        self.opp_color = Color.RED  # opponent color
    ...
    def _safe_position_score(self, state):
        """Compute position score with error handling."""
        try:
    ...
    def step(self, action):
        ...
        # ---- Channel 1: Resource Flow (snapshot) ----
        r_resource = resource_flow_reward(state, self.p0_color)

        # ---- Channel 2: Network Position (delta) ----
        current_position = self._safe_position_score(state)
        r_position = current_position - self._prev_position_score
        self._prev_position_score = current_position

        # ---- Channel 3: VP Proximity (event) ----
...
def make_env():
    ...
    env = gymnasium.make(
        ...
        config={
            ...
            "vps_to_win": 10,          # standard VP target
            "enemies": [
                WeightedRandomPlayer(Color.RED),
            ],
            # reward_function omitted — CatanRewardWrapper overrides the reward anyway
        },
    )
    # Wrap with our custom reward
    env = CatanRewardWrapper(env)
    # Wrap with action masking for MaskablePPO
...
def train(
    ...
    Args:
        ...
        eval_freq: How often (in timesteps) to run evaluation.
        n_eval_episodes: Number of games per evaluation round.
    """
